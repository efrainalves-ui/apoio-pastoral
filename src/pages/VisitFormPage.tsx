...
export function VisitFormPage() {
  ...
  return <div className="page-stack"><Link className="text-link back-link" to="/app/visitas"><ArrowLeft />Voltar às visitas</Link><header className="page-hero"><div><h1>Registrar visita pastoral</h1></div><LockKeyhole /></header>{error && <div className="alert alert--error" role="alert">{error}</div>}<form onSubmit={(event) => void submit(event)} className="visit-form">
    ...
    <Card eyebrow="5 · Última etapa" title="Pedido de oração"><label className="field"><span className="field__label">Pedido opcional</span><textarea className="field__input" rows={3} maxLength={1000} value={prayerText} onChange={(event) => setPrayerText(event.target.value)} /></label><Field label="Revisar em" name="prayer-review" type="date" value={prayerReviewAt} onChange={(event) => setPrayerReviewAt(event.target.value)} /></Card>
