...
import { quandoFoi, respostasAnteriores, visitasDaPessoa } from '../care/respostasAnteriores'
import { FOLLOW_UP_KINDS, FOLLOW_UP_LABELS, VISIT_REASONS, VISIT_REASON_LABELS, type FollowUpKind, type VisitAnswer, type VisitCompletionInput, type VisitEntity, type VisitParticipant } from '../care/types'
...
export function VisitFormPage() {
  ...
  const [nextCareUrgente, setNextCareUrgente] = useState(false)
  ...
  const [visitasAnteriores, setVisitasAnteriores] = useState<VisitEntity[]>([])
  ...
  const anteriores = useMemo(() => respostasAnteriores(visitasAnteriores, visitId ?? null), [visitasAnteriores, visitId])
  const jaVisitados = useMemo(() => membrosEscolhidos
    .map((person) => ({ person, visitas: visitasDaPessoa(visitasAnteriores, person.id, visitId ?? null) }))
    .filter(({ visitas }) => visitas.length > 0), [membrosEscolhidos, visitasAnteriores, visitId])
  ...
  return <div className="page-stack"><Link className="text-link back-link" to="/app/visitas"><ArrowLeft />Voltar às visitas</Link><header className="page-hero"><div><h1>{editando ? 'Corrigir visita' : 'Registrar visita pastoral'}</h1></div></header>{error && <div className="alert alert--error" role="alert">{error}</div>}<form onSubmit={(event) => void submit(event)} className="visit-form">
    <Card eyebrow="Contexto" title="Igreja e quem você visitou"><div className="form-grid"><label className="field" htmlFor="visit-church"><span className="field__label">Igreja</span><select id="visit-church" className="field__input" value={churchId} disabled={editando} onChange={(event) => changeChurch(event.target.value)}><option value="">Selecionar…</option>{churches.map((church) => <option key={church.id} value={church.id}>{church.name}</option>)}</select></label><label className="field"><span className="field__label">Motivo</span><select className="field__input" value={reason} onChange={(event) => setReason(event.target.value as VisitCompletionInput['reason'])}>{VISIT_REASONS.map((item) => <option key={item} value={item}>{VISIT_REASON_LABELS[item]}</option>)}</select></label><Field label="Horário da visita" name="visit-start" type="datetime-local" hint={`Duração de ${VISIT_DURATION_MINUTES} minutos, contada a partir daqui.`} value={startAt} onChange={(event) => setStartAt(event.target.value)} /></div>
      ...
      {/*
        Quem já foi visitado aparece aqui antes de a entrevista começar. É a
        diferença entre registrar mais um retrato e registrar o retrato
        seguinte — a segunda visita só significa alguma coisa ao lado da
        primeira.
      */}
      {jaVisitados.length > 0 && <ul className="ja-visitados">{jaVisitados.map(({ person, visitas }) => <li key={person.id}>
        <strong>{person.name}</strong>
        <small>{visitas.length === 1 ? 'Visitado 1 vez' : `Visitado ${visitas.length} vezes`} · última em {quandoFoi(visitas[0]!.data)}</small>
    ...
    {!editando && <Card eyebrow="Opcional" title="Vínculos e convidado"><div className="form-grid"><label className="field"><span className="field__label">Agendamento vinculado</span><select className="field__input" value={scheduledEventId} onChange={(event) => setScheduledEventId(event.target.value)}><option value="">Visita espontânea</option>{events.map((item) => <option key={item.id} value={item.id}>{item.title}</option>)}</select></label></div><div className="inline-form"><Field label="Nome" name="guest-name" value={guestName} onChange={(event) => setGuestName(event.target.value)} /><Button type="button" variant="secondary" onClick={addGuest}>Adicionar</Button></div>{guests.length > 0 && <div className="participant-grid">{guests.map((guest) => <label key={guest.id}><input type="checkbox" checked readOnly /><span>{guest.guestName}<small>Convidado não cadastrado</small></span></label>)}</div>}</Card>}
    {mode === 'full' && <Card eyebrow="Entrevista" title="Perguntas">{questionTargets().length === 0
      ...
      <div className="question-bank">{OFFICIAL_QUESTIONS.filter(({ code }) => code !== 'ORA-01').map((question) => {
        ...
        anteriorPara={(targetId) => anteriores.get(`${question.code}:${targetId}`)}
    ...
    <Card eyebrow="Anotações" title="Resumo da visita"><label className="field" htmlFor="visit-notes"><span className="field__label">Observações pastorais</span><textarea id="visit-notes" className="field__input" rows={4} maxLength={2000} value={notes} onChange={(event) => setNotes(event.target.value)} /></label></Card>
    ...
    {!editando && <Card eyebrow="Opcional" title="Pedido de oração"><label className="field" htmlFor="visit-prayer"><span className="field__label">Pedido</span><textarea id="visit-prayer" className="field__input" rows={3} maxLength={1000} value={prayerText} onChange={(event) => setPrayerText(event.target.value)} /></label><Field label="Revisar em" name="prayer-review" type="date" value={prayerReviewAt} onChange={(event) => setPrayerReviewAt(event.target.value)} /></Card>}
    {!editando && <Card eyebrow="Opcional" title="Qual é o próximo cuidado?"><label className="field"><span className="field__label">Próxima ação</span><select className="field__input" value={nextCareType} onChange={(event) => setNextCareType(event.target.value as typeof nextCareType)}><option value="">Nenhuma por enquanto</option><option value="follow_up">Criar acompanhamento</option><option value="task">Criar tarefa pastoral</option></select></label>{nextCareType === 'follow_up' && <label className="field"><span className="field__label">Tipo de acompanhamento</span><select className="field__input" value={nextCareKind} onChange={(event) => setNextCareKind(event.target.value as FollowUpKind)}>{FOLLOW_UP_KINDS.map((kind) => <option key={kind} value={kind}>{FOLLOW_UP_LABELS[kind]}</option>)}</select></label>}{nextCareType === 'task' && <><Field label="Tarefa" name="next-care-title" value={nextCareTitle} onChange={(event) => setNextCareTitle(event.target.value)} required /><label className="marca-urgente"><input type="checkbox" checked={nextCareUrgente} onChange={(event) => setNextCareUrgente(event.target.checked)} /><span>Urgente</span></label></>}{nextCareType && <><Field label="Data" name="next-care-date" type="date" value={nextCareDate} onChange={(event) => setNextCareDate(event.target.value)} required /><label className="field"><span className="field__label">Observação opcional</span><textarea className="field__input" rows={3} maxLength={2000} value={nextCareNotes} onChange={(event) => setNextCareNotes(event.target.value)} /></label></>}</Card>}
