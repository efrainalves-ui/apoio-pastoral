...
export function VisitFormPage() {
  ...
  return <div className="page-stack"><Link className="text-link back-link" to="/app/visitas"><ArrowLeft />Voltar às visitas</Link><header className="page-hero"><div><h1>Registrar visita pastoral</h1></div><LockKeyhole /></header>{error && <div className="alert alert--error" role="alert">{error}</div>}<form onSubmit={(event) => void submit(event)} className="visit-form">
    ...
    {mode === 'full' && <Card eyebrow="3 · Perguntas opcionais" title="Perguntas">{questionTargets().length === 0
      ...
      : <>{questionTargets().length > 1 && <label className="confirmation-check">
        <input type="checkbox" checked={respostaConjunta} onChange={(event) => setRespostaConjunta(event.target.checked)} />
        {/*
          A escolha é da visita, não de cada pergunta: vinte perguntas seriam
          vinte decisões. E ela não junta as respostas — cada pessoa continua
          tendo a sua, gravada com o identificador dela.
        */}
        <span>Mesma resposta para todos<small>Responda uma vez e vale para os {questionTargets().length} presentes. Desmarque a qualquer momento para responder um a um.</small></span>
      </label>}
      <div className="question-bank">{OFFICIAL_QUESTIONS.filter(({ code }) => code !== 'ORA-01').map((question) => <QuestionCard
        key={question.code}
        question={question}
        targets={questionTargets()}
        sameForAll={respostaConjunta}
