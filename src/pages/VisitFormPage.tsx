...
export function VisitFormPage() {
  ...
  return <div className="page-stack"><Link className="text-link back-link" to="/app/visitas"><ArrowLeft />Voltar às visitas</Link><header className="page-hero"><div><h1>Registrar visita pastoral</h1></div><LockKeyhole /></header>{error && <div className="alert alert--error" role="alert">{error}</div>}<form onSubmit={(event) => void submit(event)} className="visit-form">
    ...
    {mode === 'full' && <Card eyebrow="3 · Perguntas opcionais" title="Perguntas">{questionTargets().length === 0
      ...
      : <div className="question-bank">{OFFICIAL_QUESTIONS.filter(({ code }) => code !== 'ORA-01').map((question) => <QuestionCard
        key={question.code}
        question={question}
