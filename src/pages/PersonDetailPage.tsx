...
export function PersonDetailPage() {
  ...
  return <div className="page-stack person-profile"><Link className="text-link back-link" to="/app/pessoas"><ArrowLeft />Voltar às pessoas</Link><header className="page-hero district-hero"><div><p className="eyebrow">Perfil privado</p><h1>{person.name}</h1><p>{churchName} · {PASTORAL_STATUS_LABELS[person.pastoralStatus]}</p></div><div className="page-actions"><Link className="button button--primary" to={`/app/pessoas/${person.id}/editar`}><Pencil />Editar</Link><Button variant="secondary" icon={<FileText />} onClick={() => void exportar()}>Exportar dados</Button></div></header>{error && <div className="alert alert--error" role="alert">{error}</div>}
    <section className="manchete" aria-label="Idade">
      <span className="manchete__num">{calculateAge(person.birthDate) === null ? '—' : <CountUp value={calculateAge(person.birthDate)!} />}</span>
      <p className="manchete__txt">{calculateAge(person.birthDate) === null ? 'idade não informada' : 'anos'}</p>
    </section>
    ...
    <Card eyebrow="Histórico essencial" title="Alterações"><ol className="history-list">{[...person.history].reverse().map((entry) => <li key={entry.id}><span className="history-list__marker"><Clock3 /></span><div><strong>{entry.event === 'created' ? 'Pessoa cadastrada' : entry.event === 'church_changed' ? 'Igreja alterada' : entry.event === 'pastoral_status_changed' ? `Situação alterada de ${entry.from} para ${entry.to}` : entry.event === 'import_status_changed' ? `Lista: ${entry.from} → ${entry.to}` : entry.event === 'fidelity_updated' ? `Fidelidade: ${entry.from} → ${entry.to}` : entry.event === 'records_linked' ? `Cadastros vinculados: ${entry.to}` : `Dados atualizados${entry.changedFields?.length ? `: ${entry.changedFields.join(', ')}` : ''}`}</strong><small>{new Intl.DateTimeFormat('pt-BR', { dateStyle: 'medium', timeStyle: 'short' }).format(new Date(entry.at))}{entry.source ? ` · ${entry.source}` : ''}</small></div></li>)}</ol></Card>
    <section className="faixa person-profile__management" aria-label="Gerenciar dados pessoais"><h2 className="rotulo-secao">Dados pessoais</h2><div className="page-actions"><Button variant="danger" icon={<Trash2 />} onClick={() => { setPlano(null); setConfirmDelete(true); void prepararPlano() }}>Apagar dados</Button></div>{confirmDelete && <Card className="danger-card"><h2>Apagar os dados desta pessoa?</h2>
    ...
    <div className="form-actions"><Button variant="danger" onClick={() => { if (window.confirm('Apagar definitivamente os dados desta pessoa?')) void removerTudo() }}>Confirmar exclusão</Button><Button variant="secondary" onClick={() => setConfirmDelete(false)}>Cancelar</Button></div></Card>}</section>
  </div>
}
