...
export function BackupPage() {
  ...
  return <div className="page-stack page-narrow">
    ...
    <Card title="Criar backup protegido" eyebrow="Arquivo local"><p>A criação só acontece quando você clicar no botão. O arquivo não inclui o Orçamento Familiar, que deve ser exportado separadamente na própria área.</p><Button onClick={() => void create()} icon={<Download />}>Criar e salvar backup</Button>{summary && <small>Gerado em {new Intl.DateTimeFormat('pt-BR', { dateStyle: 'short', timeStyle: 'short' }).format(new Date(summary.createdAt))} · {summary.recordCount} registros · {Math.ceil(summary.size / 1024)} KB</small>}</Card>
