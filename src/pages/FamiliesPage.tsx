...
export function FamiliesPage() {
  ...
  useReloadOnSync(load); const filtered = useMemo(() => families.filter(({ name }) => normalizePersonName(name).includes(normalizePersonName(query))), [families, query]); const churchName = (id: string) => churches.find((church) => church.id === id)?.name ?? 'Igreja'
  if (loading) return <div className="app-loading">Abrindo as famílias…</div>
  return <div className="page-stack"><header className="page-hero"><div><p className="eyebrow">Famílias</p><h1>Famílias</h1></div><Link className="button button--primary" to="/app/familias/nova"><Plus />Nova família</Link></header>{error && <div className="alert alert--error">{error}</div>}<section className="district-metrics"><div><small>Famílias formadas</small><strong>{families.length}</strong></div><div><small>Pessoas organizadas</small><strong>{new Set(families.flatMap(({ memberIds }) => memberIds)).size}</strong></div><div><small>Preparação para visitas</small><strong>Pronta</strong></div></section><Card eyebrow="Organização" title="Composições"><label className="field search-only"><span className="field__label">Pesquisar família</span><span className="search-input"><Search /><input value={query} onChange={(event) => setQuery(event.target.value)} /></span></label>{filtered.length === 0 ? <div className="empty-state"><UsersRound /><strong>{families.length ? 'Nenhuma família encontrada' : 'Nenhuma família formada'}</strong><span>Crie uma família e selecione pessoas já cadastradas.</span></div> : <div className="church-grid">{filtered.map((family) => <Link className="church-card family-card" key={family.id} to={`/app/familias/${family.id}`}><span className="church-card__icon"><UsersRound /></span><h3>{family.name}</h3><p>{churchName(family.primaryChurchId)}</p><span className="church-card__address">{family.memberIds.length} {family.memberIds.length === 1 ? 'integrante' : 'integrantes'}</span></Link>)}</div>}</Card></div>
}
