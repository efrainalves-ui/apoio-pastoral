  ...
  return <div className="page-stack commission-config-page">
    ...
    <Card title="Igreja e quóruns">
      <div className="form-grid">
        <label className="field"><span className="field__label">Igreja</span><select className="field__input" value={churchId} onChange={(event) => setChurchId(event.target.value)}>{churches.map((church) => <option key={church.id} value={church.id}>{church.name}</option>)}</select></label>
        <label className="field"><span className="field__label">Ano eclesiástico</span><input className="field__input" type="number" value={year} onChange={(event) => setYear(Number(event.target.value))} /></label>
        <label className="field"><span className="field__label">Quórum da Comissão Diretiva</span><input className="field__input" type="number" min="1" value={boardQuorum || ''} onChange={(event) => setBoardQuorum(Number(event.target.value))} /></label>
        <label className="field"><span className="field__label">Quórum da Reunião Administrativa</span><input className="field__input" type="number" min="1" value={administrativeQuorum || ''} onChange={(event) => setAdministrativeQuorum(Number(event.target.value))} /></label>
      </div>
    ...
    <Card title="Pessoas responsáveis">
      ...
      <div className="checkbox-grid">{options.map((person) => <label className="choice-card" key={person.id}><input type="checkbox" checked={members.includes(person.id)} onChange={() => setMembers(members.includes(person.id) ? members.filter((id) => id !== person.id) : [...members, person.id])} /><span>{person.name}</span></label>)}</div>
      <div className="form-grid">
        <label className="field"><span className="field__label">Presidente</span><select className="field__input" value={president} onChange={(event) => setPresident(event.target.value)}><option value="">Selecionar</option>{options.map((person) => <option key={person.id} value={person.id}>{person.name}</option>)}</select></label>
        <label className="field"><span className="field__label">Secretário(a)</span><select className="field__input" value={secretary} onChange={(event) => setSecretary(event.target.value)}><option value="">Selecionar</option>{options.map((person) => <option key={person.id} value={person.id}>{person.name}</option>)}</select></label>
      </div>
      <Button disabled={busy} onClick={() => void save()}>{busy ? 'Salvando…' : 'Salvar configuração'}</Button>
    </Card>
  </div>
}
