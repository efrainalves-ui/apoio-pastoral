...
export function PrayerRequestsPage({ embedded = false }: { embedded?: boolean } = {}) {
  ...
  const formulario = draft && <Card title={editingId ? 'Editar pedido' : 'Novo pedido'}><form onSubmit={save}>
    ...
    <label className="field" htmlFor="prayer-description"><span className="field__label">Descrição curta (opcional)</span><textarea id="prayer-description" className="field__input" rows={3} value={draft.description} onChange={(event) => setDraft({ ...draft, description: event.target.value })} maxLength={1000} /><small className="field__hint">Escreva o suficiente para orar. Evite detalhes íntimos.</small></label>
