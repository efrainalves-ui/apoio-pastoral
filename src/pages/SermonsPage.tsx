...
export function SermonsPage() {
  ...
  return <div className="page-stack sermons-page"><header className="page-hero"><div><p className="eyebrow">Sermões</p><h1>Sermões e pregações</h1></div><div className="page-actions"><Button variant="secondary" icon={<FileText />} onClick={() => setRelatorioAberto((valor) => !valor)}>Histórico de pregações</Button><Link className="button" to="/app/sermoes/novo"><Plus />Novo sermão</Link></div></header>{error && <div className="alert alert--error" role="alert">{error}</div>}<section className="manchete" aria-label="Sermões na biblioteca"><span className="manchete__num"><CountUp value={sermons.length} /></span><p className="manchete__txt">{sermons.length === 1 ? 'sermão na biblioteca' : 'sermões na biblioteca'}</p></section>{relatorioAberto && <Card eyebrow="Relatório local" title="Histórico de pregações"><label className="confirmation-check"><input type="checkbox" checked={comNomes} onChange={(event) => setComNomes(event.target.checked)} /><span><strong>Incluir nomes neste relatório</strong><small>Sem marcar, saem data e igreja. Com nomes, o título da pregação entra e pode conter dados pessoais: compartilhe com cuidado.</small></span></label><Button icon={<FileText />} onClick={() => previewLocalPdf('Sermões e Pregações', sermonReportLines(preachings, sermons.length, (id) => churches.find((church) => church.id === id)?.name ?? 'Distrito', comNomes))}>Gerar histórico</Button></Card>}<Card className="sermons-catalogue"><h2 className="rotulo-secao">Biblioteca</h2><div className="filter-bar"><label className="field search-only"><span className="field__label">Pesquisar</span><span className="search-input"><Search /><input value={query} onChange={(event) => setQuery(event.target.value)} /></span></label><label className="field"><span className="field__label">Situação</span><select className="field__input" value={status} onChange={(event) => setStatus(event.target.value as SermonStatus | '')}><option value="">Todas</option>{Object.entries(SERMON_STATUS_LABELS).map(([value, label]) => <option key={value} value={value}>{label}</option>)}</select></label></div><p className="muted">{visible.length} {visible.length === 1 ? "sermão encontrado" : "sermões encontrados"}</p>{!visible.length ? <div className="empty-state"><BookOpen /><strong>Nenhum sermão encontrado</strong><Link className="button" to="/app/sermoes/novo">Criar sermão</Link></div> : <div className="sermon-list">{visible.map((sermon) => <article className="sermon-row" key={sermon.id}>
