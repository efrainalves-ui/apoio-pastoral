...
export function CommunityGroupsPage({ embutida = false }: { embutida?: boolean } = {}) {
  ...
  const { relatorios: relatoriosIntegrados } = useRelatorioIntegrado()
  ...
  /*
    A Escola Sabatina reúne as quatro prioridades, então a página fica neutra e
    cada informação leva a sua: PG, Unidade de Ação e integração são
    Discipulado; turmas de crianças, adolescentes e jovens, Novas Gerações.
  */
  const discipulado = <MarcaDaArea area="discipleship" compacta />
  const marcaDaClasse = (item: SabbathClassEntity) => item.ageGroup === 'young' || item.ageGroup === 'teenagers' || item.ageGroup === 'preteenagers' || ehClasseInfantil(item.name ?? '')
    ? <MarcaDaArea area="new_generations" compacta />
    : null

  function row(icon: string, title: string, detail: string, id: string, edit: () => void, marca: ReactNode = null) {
