...
export function NewDistrictPage() {
  ...
  /**
   * O código do backup é do pastor. Antes havia um fixo no código-fonte, o que
   * transformava o arquivo "cifrado" em texto aberto para qualquer pessoa que
   * lesse este repositório.
   */
  async function createBackup() {
    if (!account || !masterKey) return
    setError('')
    if (code.length < 12) { setError('Crie um código de backup com pelo menos 12 caracteres antes de gerar o arquivo.'); return }
    try {
      const item = await backup.create(account.id, masterKey, code)
