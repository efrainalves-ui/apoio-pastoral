...
const care = new CareService(); const families = new FamilyService(); const people = new PeopleService(); const casamentos = new CasamentoService()
export function VisitDetailPage() {
  ...
  const load = useCallback(async () => { if (!account || !masterKey) return; const current = await care.getVisit(account.id, masterKey, visitId); setVisit(current); if (!current) return; const casamento = current.casamentoId ? await casamentos.obter(account.id, masterKey, current.casamentoId) : null; if (!current.targetId && casamento) { setTargetName(nomeDoCasal(casamento)); return } const target = current.targetType === 'family' ? await families.getFamily(account.id, masterKey, current.targetId) : await people.getPerson(account.id, masterKey, current.targetId); setTargetName(target?.name ?? 'Cadastro preservado'); }, [account, masterKey, visitId])
