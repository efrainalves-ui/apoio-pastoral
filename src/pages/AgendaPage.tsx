...
export function AgendaPage() {
  const { account, masterKey } = useAuthVault(); const [events, setEvents] = useState<AgendaEventEntity[]>([]); const [churches, setChurches] = useState<ChurchEntity[]>([]); const [view, setView] = useState<AgendaView>('week'); const [anchor, setAnchor] = useState(() => new Date()); const [selected, setSelected] = useState<Set<string>>(new Set()); const [dia, setDia] = useState<string | null>(null); const [comNomes, setComNomes] = useState(false); const [pdfOpen, setPdfOpen] = useState(false); const [error, setError] = useState('')
  ...
  function renderDias(dias: Date[], comVazios: boolean) {
    const mostrados = comVazios ? dias : dias.filter((day) => eventosDoDia(day).length > 0)
    if (!mostrados.length) return <div className="empty-state"><CalendarDays /><strong>Nenhum compromisso neste período</strong><Link className="button" to={newEvent(dias[0] ?? anchor)}><Plus />Criar compromisso</Link></div>
