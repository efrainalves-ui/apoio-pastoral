...
import { agendaText, canDeliberate, isTie, presidentMayBreakTie, reorderAgenda, requiredMajority, tieBreakNote } from '../commissions/core'
import { CommissionService } from '../commissions/service'
import type { PresidentTieBreak } from '../commissions/types'
...
export function CommissionMeetingPage() {
  ...
  const [votes, setVotes] = useState<Record<string, { favorable: string; against: string; abstentions: string; presidentVoted: boolean; presidentTieBreak: PresidentTieBreak }>>({})
  ...
  async function vote(entry: CommissionAgendaItem) {
    if (!account || !masterKey || !meeting) return
    const draft = votes[entry.id] ?? { favorable: '0', against: '0', abstentions: '0', presidentVoted: false, presidentTieBreak: null }
    await run(async () => { setMeeting(await service.confirmVote(account.id, masterKey, meeting.id, entry.id, Number(draft.favorable), Number(draft.against), Number(draft.abstentions), draft.presidentVoted, draft.presidentTieBreak)) }, 'Votação confirmada e registrada na ata.')
  }
  ...
  return <div className="page-stack commission-print-root">
    ...
    {tab === 'pauta' && <>
      ...
      <Card title="Ordem dos assuntos">{meeting.agenda.length ? <div className="entity-list">{meeting.agenda.map((entry, index) => {
        const draft = votes[entry.id] ?? { favorable: '0', against: '0', abstentions: '0', presidentVoted: false, presidentTieBreak: null }
        const empatado = isTie(Number(draft.favorable), Number(draft.against))
        const podeDesempatar = presidentMayBreakTie(Number(draft.favorable), Number(draft.against), draft.presidentVoted)
        return <article className="entity-row commission-agenda-item" key={entry.id}><div><strong>{entry.order}. {entry.title}</strong><small>{decisionLabels[entry.decisionType]} · {destinationLabels[entry.destination]}{entry.department ? ` · ${entry.department}` : ''}</small>{entry.sourceVoteNumber && <small>Voto de origem: {entry.sourceVoteNumber}</small>}<p>{agendaText(entry)}</p>{entry.responsibleId && <small>Responsável: {personName(entry.responsibleId)}{entry.dueDate ? ` · prazo ${entry.dueDate}` : ''}</small>}
          {entry.vote ? <div className="decision-result"><strong>{entry.vote.voteNumber ?? 'Decisão sem número'} · {resultLabels[entry.vote.result]}</strong><span>{entry.vote.favorable} favoráveis · {entry.vote.against} contrários · {entry.vote.abstentions} abstenções</span>{entry.vote.presidentTieBreak && <small>{tieBreakNote(entry.vote.presidentTieBreak)}</small>}<label className="field"><span className="field__label">Texto final da ata</span><textarea className="field__input field__textarea" disabled={frozen} value={entry.vote.finalText} onChange={(event) => updateFinalText(entry, event.target.value)} /></label></div> : <div className="vote-grid"><label className="field"><span className="field__label">Favoráveis</span><input className="field__input" type="number" min="0" value={draft.favorable} onChange={(event) => setVotes({ ...votes, [entry.id]: { ...draft, favorable: event.target.value } })} /></label><label className="field"><span className="field__label">Contrários</span><input className="field__input" type="number" min="0" value={draft.against} onChange={(event) => setVotes({ ...votes, [entry.id]: { ...draft, against: event.target.value } })} /></label><label className="field"><span className="field__label">Abstenções</span><input className="field__input" type="number" min="0" value={draft.abstentions} onChange={(event) => setVotes({ ...votes, [entry.id]: { ...draft, abstentions: event.target.value } })} /></label><label className="choice-card commission-president-vote"><input type="checkbox" checked={draft.presidentVoted} onChange={(event) => setVotes({ ...votes, [entry.id]: { ...draft, presidentVoted: event.target.checked, presidentTieBreak: null } })} /><span>O presidente já votou nesta contagem</span></label>{empatado && <div className="commission-tiebreak">{podeDesempatar ? <><small>A votação empatou. O presidente pode desfazer o empate com um voto de qualidade.</small><label className="field"><span className="field__label">Voto de desempate do presidente</span><select className="field__input" value={draft.presidentTieBreak ?? ''} onChange={(event) => setVotes({ ...votes, [entry.id]: { ...draft, presidentTieBreak: (event.target.value || null) as PresidentTieBreak } })}><option value="">Sem voto de desempate</option><option value="favorable">A favor</option><option value="against">Contra</option></select></label></> : <small>A votação empatou e o presidente já votou, então o assunto fica adiado para a próxima reunião.</small>}</div>}<div><small>Maioria necessária: {requiredMajority(Number(draft.favorable) + (draft.presidentTieBreak === 'favorable' ? 1 : 0), Number(draft.against) + (draft.presidentTieBreak === 'against' ? 1 : 0))}</small><Button disabled={busy || frozen || (!hasQuorum && entry.decisionType !== 'record')} onClick={() => void vote(entry)}>Confirmar votação</Button></div></div>}
