...
export function CommissionMeetingPage() {
  ...
  const agendaDoc = useMemo(() => meeting ? service.agendaDocument(meeting, churchName, nomeNoDocumento, comNomes) : '', [meeting, churchName, nomeNoDocumento, comNomes])
  const minutesDoc = useMemo(() => meeting ? service.minutesDocument(meeting, churchName, quorum, nomeNoDocumento, comNomes) : '', [meeting, churchName, quorum, nomeNoDocumento, comNomes])
  ...
  return <div className="page-stack commission-print-root">
    ...
    {tab === 'preparar' && <>
      ...
      <Card title="Participantes"><div className="checkbox-grid" role="group" aria-label="Participantes da reunião">{memberOptions.map((person) => <label className="choice-card" key={person.id}><input type="checkbox" disabled={frozen} checked={meeting.participantIds.includes(person.id)} onChange={() => toggleParticipant(person.id)} /><span>{person.name}</span></label>)}</div><div className="form-grid"><label className="field"><span className="field__label">Convidados</span><textarea className="field__input" disabled={frozen} placeholder="Um nome por linha" value={meeting.guestNames.join('\n')} onChange={(event) => change({ guestNames: event.target.value.split('\n').map((value) => value.trim()).filter(Boolean) })} /></label><label className="field"><span className="field__label">Convidados com voz e voto</span><textarea className="field__input" disabled={frozen} placeholder="Um nome por linha" value={meeting.votingGuestNames.join('\n')} onChange={(event) => change({ votingGuestNames: event.target.value.split('\n').map((value) => value.trim()).filter(Boolean) })} /></label></div></Card>
      ...
      <Card className="commission-printable" title="Pauta para imprimir" action={<FileText />}><div className="no-print"><IncludeNames checked={comNomes} onChange={setComNomes} detalhe="Com nomes, entram presidente, secretário, participantes, convidados, local, oração, reflexão, os assuntos, as propostas e as observações." /></div><pre className="preserved-text commission-document">{agendaDoc}</pre><div className="form-actions no-print"><Button variant="secondary" onClick={() => void copy(agendaDoc)} icon={<Copy />}>Copiar pauta</Button><Button variant="secondary" onClick={() => window.print()} icon={<Printer />}>Imprimir / salvar em PDF</Button></div></Card>
    </>}
    ...
    {tab === 'ata' && <>
      {!minutesReady ? <Card title="Ata ainda não pode ser gerada"><p className="muted">{meeting.agenda.length ? `Registre a decisão de ${pending.length} assunto(s) na etapa 2 antes de gerar a ata.` : 'Prepare a pauta e realize a comissão antes de gerar a ata.'}</p><Button variant="secondary" onClick={() => setTab('realizar')}>Voltar à comissão</Button></Card> : <>
        ...
        <Card className="commission-printable" title="Ata da reunião"><div className="no-print"><IncludeNames checked={comNomes} onChange={setComNomes} detalhe="Com nomes, entram presidente, secretário, participantes, convidados, local, oração, reflexão, os assuntos, as propostas e as observações." /></div><pre className="preserved-text commission-document">{minutesDoc}</pre><div className="form-actions no-print"><Button variant="secondary" onClick={() => void copy(minutesDoc)} icon={<Copy />}>Copiar ata</Button><Button variant="secondary" onClick={() => window.print()} icon={<Printer />}>Imprimir / salvar em PDF</Button>{!frozen && <Button disabled={busy} onClick={() => void finalize()}>Finalizar ata</Button>}</div></Card>
      </>}
    </>}
