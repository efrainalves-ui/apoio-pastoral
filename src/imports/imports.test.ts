...
describe('importadores locais e idempotentes', () => {
  ...
  it('classifica fidelidade por meses, informa divergências e descarta campos não necessários', async () => {
    ...
    const preview = await imports.previewFidelity(accountId, masterKey, 'hash-fidelity', rows, [person], churches)
    ...
    expect(updated?.fidelity).toMatchObject({ months: 9, category: 'tither', source: 'PDF local de fidelidade' })
  ...
  it('preserva a faixa sem inventar quantidade exata', async () => {
    ...
    const preview = await imports.previewFidelity(accountId, masterKey, 'hash-range', rows, [person], churches)
  ...
  it('classifica faixas e categoria sem inventar meses e deixa somente ambiguidades reais pendentes', async () => {
    ...
    const preview = await imports.previewFidelity(accountId, masterKey, 'hash-official-categories', rows, [first, second], churches)
  ...
  it('mantém linha repetida pendente sem impedir a associação segura original', async () => {
    ...
    const preview = await imports.previewFidelity(accountId, masterKey, 'hash-duplicate-row', rows, [person], churches)
    expect(preview.changes).toHaveLength(1)
  ...
  it('sinaliza nome semelhante sem criar associação automática', async () => {
    ...
    const preview = await imports.previewFidelity(accountId, masterKey, 'hash-similar', rows, [person], churches)
