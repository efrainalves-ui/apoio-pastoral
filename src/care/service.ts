...
function assertCompletion(input: VisitCompletionInput): void {
  // A visita de casamento pode ser a noivos que não são membros: vale pelo acompanhamento, sem pessoa cadastrada.
  if (!input.churchId || (!input.targetId && !input.casamentoId)) throw new Error('Selecione a pessoa ou família e a igreja da visita.')
  if (!input.casamentoId && !input.participants.some(({ present }) => present)) throw new Error('Informe ao menos uma pessoa presente.')
...
export class CareService {
  ...
  private async list<T extends CareEntity>(accountId: string, masterKey: CryptoKey, recordType: VaultRecord['recordType'], payloadType: string): Promise<T[]> {
    const values: T[] = []
    for (const record of await this.repository.list(accountId, recordType)) { const payload = await readPayload(masterKey, record, this.database); if (payload?.type === payloadType) { const value = decodeData<T>(record, payload.data); if (value) values.push(value) } }
    return values
  }
  ...
  async completeVisit(accountId: string, masterKey: CryptoKey, input: VisitCompletionInput): Promise<VisitEntity> {
    ...
    const visit: VisitData = { targetType: input.targetType, targetId: input.targetId, churchId: input.churchId, scheduledEventId: input.scheduledEventId, ...(input.casamentoId ? { casamentoId: input.casamentoId } : {}), mode: input.mode, status: 'completed', currentVersion: 1, versions: [version], createdAt: now, updatedAt: now }
