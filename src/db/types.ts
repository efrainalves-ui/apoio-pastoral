...
export interface VaultRecord extends CipherEnvelope {
  id: string
  accountId: string
  recordType: 'foundation_fixture' | 'district' | 'church' | 'person' | 'family' | 'import_batch' | 'agenda_event' | 'sermon' | 'sermon_preaching' | 'goal' | 'goal_entry' | 'goal_history' | 'visit' | 'prayer_request' | 'follow_up' | 'task' | 'visit_round' | 'interest' | 'bible_study' | 'missionary_pair' | 'sabbath_class' | 'small_group' | 'uapg' | 'commission_config' | 'commission_meeting' | 'commission_task' | 'nomination_process' | 'annual_goal' | 'evangelism_campaign' | 'work_allowance' | 'work_expense' | 'mileage' | 'work_config' | 'work_dependent' | 'work_entry' | 'letra_budget' | 'letra_item' | 'letra_acquisition' | 'work_paycheck' | 'work_trip' | 'personal_reading_book' | 'personal_reading_session' | 'personal_reading_goal' | 'personal_shopping' | 'pessoal_lancamento' | 'pessoal_transferencia' | 'pessoal_conta' | 'pessoal_cartao' | 'pessoal_integrante' | 'pessoal_meta' | 'pessoal_aporte' | 'pessoal_planejamento' | 'pessoal_compra' | 'pessoal_migracao' | 'material' | 'material_distribution' | 'material_need' | 'acms_report' | 'integrated_report' | 'wedding' | 'reminder' | 'reminder_list' | 'reminder_meta' | 'encrypted'
  version: number
  createdAt: string
  updatedAt: string
  deletedAt?: string
