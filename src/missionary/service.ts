...
export class MissionaryService {
  ...
  private async save<T extends object>(accountId:string,key:CryptoKey,type:'sabbath_class'|'small_group'|'uapg',data:T):Promise<{id:string}&T>{const id=crypto.randomUUID();await this.repo.saveEncrypted(accountId,currentDeviceId(),id,await encryptPayload(key,{schemaVersion:1,type,data},id),type);return {id,...data} }
  ...
  async remove(accountId:string,key:CryptoKey,id:string){const record=await this.repo.list(accountId);const current=record.find(item=>item.id===id);if(!current)throw new Error('Registro não encontrado.');await this.repo.deleteEncrypted(accountId,currentDeviceId(),id,await encryptPayload(key,{schemaVersion:1,type:'removed',data:{at:new Date().toISOString()}},id))}
  async replace(accountId:string,key:CryptoKey,id:string,type:'sabbath_class'|'small_group'|'uapg',data:object){await this.repo.saveEncrypted(accountId,currentDeviceId(),id,await encryptPayload(key,{schemaVersion:1,type,data},id),type)}
  async savePair(accountId: string, key: CryptoKey, churchId: string, memberIds: string[]) {
    ...
    await this.repo.saveEncrypted(accountId, currentDeviceId(), id, await encryptPayload(key, { schemaVersion: 1, type: 'missionary_pair', data }, id), 'missionary_pair'); return { id, ...data }
  }
  async saveInterest(accountId: string, key: CryptoKey, input: Omit<InterestData, 'createdAt' | 'updatedAt'>, id: string = crypto.randomUUID()) {
    ...
    await this.repo.saveEncrypted(accountId, currentDeviceId(), id, await encryptPayload(key, { schemaVersion: 1, type: 'interest', data }, id), 'interest')
    return { id, ...data }
  }
  async startStudy(accountId: string, key: CryptoKey, interest: InterestEntity, startedAt: string) {
    ...
    await this.repo.saveEncrypted(accountId, currentDeviceId(), id, await encryptPayload(key, { schemaVersion: 1, type: 'bible_study', data }, id), 'bible_study')
  ...
  async completeStudy(accountId: string, key: CryptoKey, study: BibleStudyEntity, interest: InterestEntity, outcome: 'continue_interested' | 'baptized', followUp: string) {
    ...
    await this.repo.saveEncrypted(accountId, currentDeviceId(), study.id, await encryptPayload(key, { schemaVersion: 1, type: 'bible_study', data }, study.id), 'bible_study')
