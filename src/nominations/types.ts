...
export interface NominationOffice { id: string; area: string; title: string; vacancies: number; multiplePeople: boolean; /** Cargos de ancião e do diaconato nunca têm associados. */ allowsAssociates?: boolean; description: string; status: OfficeStatus; indicatedByOtherCommittee: boolean; officialStatus: OfficialOfficeStatus; archivedAt?: string }
...
export interface NominationCandidate { id: string; officeId: string; personId: string; /** Indicação para associado(a) do cargo, quando ele permite. */ associate?: boolean; status: CandidateStatus; confidentialNote: string; conversationDate: string; consent: boolean; refusalReason: string; eligibility: EligibilityStatus; fidelityAlert: boolean; vote?: NominationVote; electedAt?: string }
