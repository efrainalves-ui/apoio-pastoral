...
export function readingSummary(books: ReadingEntity<ReadingBookData>[], sessions: ReadingEntity<ReadingSessionData>[], goal: ReadingEntity<ReadingGoalData> | null, month: string) {
  ...
  return { reading: books.filter(({ status }) => status === 'reading').length, booksCompletedMonth, booksCompletedYear, pages, minutes, goalBooks: goal?.books ?? 0, goalPages: goal?.pages ?? 0, goalMinutes: goal?.minutes ?? 0 }
}
