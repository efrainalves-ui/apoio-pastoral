...
function fontes(atual: number, anterior: number): AreaSources {
  return {
    ...semNada,
    entries: atual > 0
      ? [{ id: 'e1', churchId: 'igreja-1', metric: 'tithes_offerings', date: '2026-03-01', amount: atual, source: 'pdf', reference: '', createdAt: '' }]
      : [],
    history: anterior > 0
      ? [{ id: 'h1', area: 'financial', year: 2025, amount: anterior, source: 'pdf', reference: '', createdAt: '', updatedAt: '' }]
      : [],
  }
}

const meta = (target: number, targetKind?: 'value' | 'percent'): GoalEntity[] => [{
  id: 'meta-1', churchId: null, year: 2026, metric: 'tithes_offerings', target,
  ...(targetKind ? { targetKind } : {}), createdAt: '', updatedAt: '',
}]

describe('meta financeira escrita em porcentagem', () => {
  it('traduz a porcentagem em número, usando o ano anterior', () => {
    // "Dez por cento a mais que 1000" é 1100 — é contra isso que se mede.
    const comparacao = areaComparison('financial', meta(10, 'percent'), fontes(550, 1000), 2026)
  ...
  it('sem ano anterior, avisa em vez de inventar progresso', () => {
    ...
    const comparacao = areaComparison('financial', meta(10, 'percent'), fontes(550, 0), 2026)
  ...
  it('meta antiga em valor continua valendo como valor, e pede para ser reescrita', () => {
    ...
    const comparacao = areaComparison('financial', meta(1000), fontes(550, 1000), 2026)
