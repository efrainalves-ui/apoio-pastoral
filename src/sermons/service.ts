...
export class SermonService {
  ...
  async list(accountId: string, masterKey: CryptoKey): Promise<SermonEntity[]> { const records = await this.repository.list(accountId, 'sermon'); const items = await Promise.all(records.map(async (record) => { const payload = await readPayload(masterKey, record, this.database); return payload?.type === 'sermon' ? { id: record.id, ...(payload.data as Omit<SermonEntity, 'id'>) } : null })); return items.filter((item): item is SermonEntity => Boolean(item)).sort((a, b) => b.updatedAt.localeCompare(a.updatedAt)) }
