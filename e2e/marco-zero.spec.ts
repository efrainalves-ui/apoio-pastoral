...
test('continua disponível offline depois do primeiro carregamento', async ({ page, context }) => {
  ...
  await page.getByRole('radio', { name: 'Departamento' }).check()
  await page.getByRole('combobox', { name: 'Departamento' }).fill('Música')
  await page.getByRole('option', { name: 'Música' }).click()
