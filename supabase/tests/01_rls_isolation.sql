...
select homologacao_testes.exigir(
  ...
  = array[
      ...
      -- Notificações. A lista é comparada em ordem alfabética, então estas
      -- cinco ficam juntas e nesta ordem. Configuração do envio (0011) e, da
      -- 0013: `disponivel` é a única que `authenticated` executa, e ela não lê
      -- dado de conta nenhuma — responde se este banco tem as tabelas e
      -- funções do push. As outras duas são só do servidor e existem para que
      -- o privilégio direto em `push_subscriptions` pudesse ser devolvido:
      -- aparelho revogado não é avisado nem por engano. Provas 05 e 06.
      'lembretes_push_config', 'lembretes_push_disponivel',
      'lembretes_push_esquecer_inscricao', 'lembretes_push_guardar_vapid',
