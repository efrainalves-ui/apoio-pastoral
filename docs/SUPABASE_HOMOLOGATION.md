...
5. Para as **notificações dos lembretes**, aplicar também `0010_lembretes_push_up.sql`, `0011_lembretes_push_config_up.sql`, `0012_lembretes_push_servidor_up.sql` e `0013_push_do_aparelho_revogado_up.sql`. Nenhuma delas muda a versão do esquema, que continua **9**. Conferir com `select public.lembretes_push_disponivel();`, que precisa responder **true** — é o que o aplicativo pergunta antes de oferecer o recurso. O restante da configuração (chaves, função de envio, agendamento) está em `docs/LEMBRETES_NOTIFICACOES.md`.
